...
public class GribManager : MonoBehaviour
{
    ...
    [SerializeField] Sprite[] SpriteEmptys;
    ...
    void GribCreate(int[,] map)
    {
        ...
        GameController.action.MaxCell = GameController.action.CellNotEmpty;
        GameController.action.UpdateTarget();
    }
        ...
        for (int x = 0; x < 7; x++)
        {
            for (int y = 0; y < 9; y++)
            {
                ...
                else
                {
                    if (x != 0 && x != 6 && y != 0 && y != 8)
                    {
                        CellEmptyPrefab.GetComponent<SpriteRenderer>().sprite = SpriteEmptys[Random.Range(0, SpriteEmptys.Length)];
                        Instantiate(CellEmptyPrefab, EmptyParent.transform).transform.localPosition = new Vector2(x, y);
                    }
