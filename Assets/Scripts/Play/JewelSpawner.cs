...
public class JewelSpawner : MonoBehaviour
{
    ...
    public SpriteAtlas JewelSprite;
    ...
    {
        ServiceLocator.Unset<JewelSpawner>();
    }
    ...
    public GameObject JewelInstantiate(int x, int y)
    {
        ...
        if (PLayerInfo.MODE == 1)
            ...
            r = Random.Range(0, 7);
        JewelScript.render.sprite = JewelSprite.GetSprite(r+"");
        if (JewelScript.render.sprite == null)
        {
    ...
    public GameObject SpawnJewelPower(int type, int power, Vector3 pos)
    {
        ...
        JewelGribScript[x, y] = JewelScript;
        if (type != 8)
            JewelScript.render.sprite = JewelSprite.GetSprite(type+ "");
        JewelScript.jewel.JewelPosition = new Vector2(x, y);
        ...
        if (power == (int)GameController.Power.BOOM)
            JewelScript.render.sprite = JewelSprite.GetSprite(type +""+ power);
