...
public class GameController : MonoBehaviour
{
    ...
    public int MaxCell;
    [SerializeField] private Image sliderTarget;
    [SerializeField] private Image sliderTargetBackground;
    [SerializeField] private Image imgStarEnd;
    ...
    private GameSkillController gameSkillController;
    void Awake()
    {
        action = this;
    }
    ...
    {
        sliderTargetBackground.gameObject.SetActive(CellNotEmpty != 0);
        sliderTarget.gameObject.SetActive(CellNotEmpty != 0);
        imgStarEnd.gameObject.SetActive(CellNotEmpty == 0);
        sliderTarget.DOFillAmount(((float)MaxCell - CellNotEmpty) / ((float)MaxCell),0.5f);
        
    }
    IEnumerator Start()
    {
        ServiceLocator.Set(this);
        gameSkillController = ServiceLocator.Get<GameSkillController>();
        ServiceLocator.GetUIViewManager.CloseAllUIView();
        ServiceLocator.GetUIViewManager.ShowUIView<UIViewActionPhase>();
    ...
    {
        ServiceLocator.Unset<GameController>();
    }
    ...
    void JewelSelecter()
    {
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (gameSkillController.IsSkillClick())
            {
                Selector.SetActive(false);
                ishold = false;
                Pointer = JewelTouchChecker(Input.mousePosition);
                if (Pointer == null) return;
                gameSkillController.DoSkill(Pointer.GetComponent<JewelObj>());
            }
        }
    ...
    public void RuleChecker(GameObject obj1, GameObject obj2)
    {
        ...
        CreateMovingObj(Jewel1);
        CreateMovingObj(Jewel2);
        ...
        {
            ServiceLocator.Get<GameSkillController>().BonusPotionDown();
            obj1.transform.DOMove(obj2.transform.position, 0.2f);
            obj2.transform.DOMove(obj1.transform.position, 0.2f);
    ...
    void CreateMovingObj(JewelObj obj)
    {
        if (obj.jewel.JewelType < 6)
        {
            var template = EffectSpawner.effect.MovingEffect[0];

            if (template != null)
            {
                GameObject tmp = (GameObject)Instantiate(template);
    ...
    public void PBoomMore(int x, int y, int moreX, int moreY)
    {
        dropjewel();
        for (int i = x - moreX; i <= x + moreX; i++)
            for (int j = y - moreY; j <= y + moreY; j++)
            {
                if (i != x || j != y)
                    if (i >= 0 && i < 7 && j >= 0 && j < 9 && JewelSpawner.spawn.JewelGribScript[i, j] != null && JewelSpawner.spawn.JewelGribScript[i, j].jewel.JewelType != 99)
                        JewelSpawner.spawn.JewelGribScript[i, j].Destroy();
    ...
    public void AddBonusPower()
    {
        ...
        while (true)
        {
            ...
            if (tmp != null && tmp.jewel.JewelType != 8 && tmp.jewel.JewelPower == 0 && GribManager.cell.GribCellObj[x, y].cell.CellEffect == 0)
            {
                ...
                EffectSpawner.effect.ReloadImagePower(tmp, r);
                return;
            }
        }
    }
